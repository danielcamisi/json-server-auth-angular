...
export class ProjetoService {
  ...
  updateProjetoStatus(projetoId: number, status: string): Observable<any> {
    const options = { 
      headers: new HttpHeaders({ 'Content-Type': 'application/json' }),
      responseType: 'text' as 'json'  // Indica que a resposta esperada é texto
    };
    return this.http.put(`${this.apiUrl}/${projetoId}`, { status }, options);
  }
}
